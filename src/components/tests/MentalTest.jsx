...
import { MENTAL_QUESTIONS } from '../../lib/mentalConfigs';
import { calculateMentalScore } from '../../lib/mentalConfigs';
import { supabase } from '../../lib/supabase';
import { loadTestData } from '../../lib/testDataLoader';

const STORAGE_KEY = 'mental-test-draft';
...
- Ton: bienveillant, analytique, actionnable, jamais condescendant
...
- Inclure des exercices de productivité mentale, gestion de la charge cognitive, pensée critique
- Recommander des livres sur l'intelligence cognitive (Kahneman, Thinking Fast and Slow, etc.)
- Proposer un type de coaching ou thérapie adapté (coaching cognitif, TCC, etc.)

IMPORTANT: Retourne UNIQUEMENT le JSON valide, sans texte avant or après, sans balises markdown.`;
...
export default function MentalTest({ user, onComplete, onCancel }) {
  ...
  const handleNext = () => {
    if (currentQuestion < MENTAL_QUESTIONS.length - 1) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Calcul des scores
      const scores = calculateMentalScore(answers);
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          test_type: 'mental', // Identifier comme test mental
          session_data: {
            type: 'mental',
        ...
        <div className="w-full max-w-2xl">
          <div className="bg-white rounded-2xl border p-10 text-center shadow-lg" style={{ borderColor: '#E8E0D8' }}>
            ...
            <p className="text-base mb-6 max-w-lg mx-auto" style={{ color: '#666' }}>
              Évalue 12 dimensions de ton fonctionnement mental : structuration, décision, focus, apprentissage, flexibilité...
            </p>
            ...
            <div className="text-left p-4 rounded-lg mb-8 text-sm" style={{ backgroundColor: '#F5F0EA', borderLeft: '3px solid #C96442', color: '#666' }}>
              <strong>Important :</strong> Ce test explore comment tu structures ta pensée, prends des décisions et gères ta charge cognitive. Sois honnête — il n'y a pas de bonne réponse, seulement ta vérité. Plus tu détailles tes réponses, plus l'analyse sera précise.
            </div>
        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white rounded-2xl border p-8 shadow-lg" style={{ borderColor: '#E8E0D8' }}>
            <TestProgress 
              ...
              total={MENTAL_QUESTIONS.length}
              percent={((currentQuestion + 1) / MENTAL_QUESTIONS.length) * 100}
            />
            ...
            <TestQuestion
              ...
              isLast={currentQuestion === MENTAL_QUESTIONS.length - 1}
            />
          </div>
        </div>
      </div>
...
function generateFallbackReport(scores) {
  ...
  return {
    profil_global: {
      ...
      synthese: `Votre profil mental montre des capacités cognitives ${scores.interpretation.toLowerCase()}. Une analyse personnalisée sera disponible prochainement.`
    },
    dynamiques: [
      {
        titre: "Analyse personnalisée en cours",
        citation: "Vos réponses sont en cours d'analyse par notre système.",
        description: "Une analyse approfondie de vos patterns de pensée et de vos processus cognitifs est en préparation."
      }
    ],
    forces: [
      {
        titre: "Curiosité pour la connaissance de soi",
        description: "Votre participation à cet audit montre une volonté réelle de mieux comprendre votre fonctionnement mental."
      },
      {
    ...
    axes_prioritaires: [
      {
        ...
        exercice: "Utiliser la méthode des 5W (Who, What, When, Where, Why) pour structurer un problème chaque jour",
        frequence: "1×/jour",
        duree: "5 minutes",
        signal_reussite: "Vous arrivez à décomposer un problème complexe en 5 questions claires"
      }
    ],
    conseils_generaux: [
      "Les capacités cognitives se développent avec la pratique régulière",
      "La pensée structurée est une compétence qui s'apprend comme une langue étrangère",
      "Soyez patient et bienveillant envers vous-même dans ce processus",
      "N'hésitez pas à consulter un coach ou thérapeute cognitif pour un accompagnement personnalisé"
    ],
    recommandations_quotidiennes: {
      matin: "Identifier une décision à prendre aujourd'hui et la structurer en 3 étapes",
      journee: "Faire une pause de 5 minutes pour observer un pattern de pensée en cours",
      soir: "Écrire 3 lignes dans un journal : problème rencontré, méthode utilisée, résultat"
    },
    ressources: {
      livre: {
        titre: "Thinking, Fast and Slow",
        auteur: "Daniel Kahneman",
        pourquoi: "Le livre fondateur sur les deux systèmes de pensée — accessible et transformateur"
      },
      concepts: [
        "Système 1 vs Système 2 (Kahneman)",
        "Biais cognitifs (confirmation, ancrage, disponibilité)",
        "Charge cognitive et mémoire de travail",
        "Pensée critique et esprit scientifique"
      ],
      praticien: "Coaching cognitif ou thérapie métacognitive pour le développement des capacités mentales"
    }
  };
}

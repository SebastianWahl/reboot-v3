...
export default function HomeScreen({ onSignInWithGoogle, onSignInWithEmail }) {
    ...
    <div className="min-h-screen bg-[#f5f0ea]">
      ...
      <section id="commencer" ref={formRef} className="py-16 lg:py-20" style={{ backgroundColor: '#C96442' }}>
        <div className="max-w-sm mx-auto px-6 text-center">
          ...
          <p className="text-sm mb-8" style={{ color: 'rgba(255,255,255,0.7)' }}>15–20 min · Résultat personnalisé · Gratuit</p>

          <div className="bg-white rounded-2xl p-6">
            {sent ? (
              <div className="py-4">
                <p className="text-sm font-semibold text-[#1a1209] mb-1">Lien envoyé !</p>
                <p className="text-xs text-[#888]">Vérifie ta boîte mail à <strong>{email}</strong>.</p>
              </div>
            ) : (
              <>
                {/* Google */}
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-3">
                  ...
                  <button
                    ...
                    style={{ backgroundColor: emailLoading ? '#ccc' : '#1a1209' }}
                  >
                    {emailLoading ? 'Envoi…' : 'Recevoir un lien de connexion'}
                  </button>
                </form>
              </>
            )}
